...
from cryptopy.event import Event, EventEngine
from cryptopy.trader.engine import BaseEngine, MainEngine
...
from cryptopy.trader.event import (
    EVENT_TICK,
    EVENT_ORDER,
    EVENT_TRADE,
    EVENT_POSITION
...
from cryptopy.trader.utility import load_json, save_json, extract_vt_symbol, round_to
from cryptopy.trader.converter import OffsetConverter
from .base import (
    APP_NAME,
    EVENT_PORTFOLIO_LOG,
...
class StrategyEngine(BaseEngine):
    ...
    def init_engine(self):
        ...
        self.register_event()
        self.write_log("Porfolio Strategy Engine Sucessfully Inited")
    ...
    def send_order(
        ...
        if not contract:
            self.write_log(f"Send order failed. No symbol.: {vt_symbol}", strategy)
            return ""
    ...
    def cancel_order(self, strategy: StrategyTemplate, vt_orderid: str):
        ...
        if not order:
            self.write_log(f"Cancel Order failed. No order. {vt_orderid}", strategy)
            return
    ...
    def call_strategy_func(
        ...
        try:
            ...
            msg = f"Trigger Exception and has stopped\n{traceback.format_exc()}"
    ...
    def add_strategy(
        ...
        if strategy_name in self.strategies:
            self.write_log(f"Fail to create strategy because of duplicated names {strategy_name}")
            return

        strategy_class = self.classes.get(class_name, None)
        if not strategy_class:
            self.write_log(f"Fail to create strategy because not found strategy class {class_name}")
            return
    ...
    def _init_strategy(self, strategy_name: str):
        ...
        if strategy.inited:
            self.write_log(f"{strategy_name} Completed init. Repeat actions are prohibited.")
            return
        ...
        for vt_symbol in strategy.vt_symbols:
            ...
            if contract:
                ...
                self.write_log(f"Fail to subscribe market data. No Symbol.{vt_symbol}", strategy)

        # Put event to update init completed status.
        strategy.inited = True
        self.put_strategy_event(strategy)
        self.write_log(f"{strategy_name} Complete init.")
    ...
    def start_strategy(self, strategy_name: str):
        ...
        if not strategy.inited:
            self.write_log(f"Strategy {strategy.strategy_name} failed to start. Please init first.")
            return

        if strategy.trading:
            self.write_log(f"{strategy_name} has started. Repeat actions are prohibited.")
            return
    ...
    def remove_strategy(self, strategy_name: str):
        ...
        strategy = self.strategies[strategy_name]
        if strategy.trading:
            self.write_log(f"Strategy {strategy.strategy_name} fail to remove. Please stop first.")
            return
    ...
    def load_strategy_class(self):
        ...
        self.load_strategy_class_from_folder(path1, "cryptopy_portfoliostrategy.strategies")
    ...
    def load_strategy_class_from_module(self, module_name: str):
        ...
        try:
            ...
            msg = f"Strategy module {module_name} fail to load and trigger Exception.:\n{traceback.format_exc()}"
            self.write_log(msg)
