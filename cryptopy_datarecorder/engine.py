...
from cryptopy.trader.constant import Exchange
...
from cryptopy.trader.event import EVENT_TICK, EVENT_CONTRACT, EVENT_TIMER
from cryptopy.trader.utility import load_json, save_json, BarGenerator
from cryptopy.trader.database import BaseDatabase, get_database
...
class RecorderEngine(BaseEngine):
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine, setting=None) -> None:
        """"""
    ...
    def load_setting(self, setting=None) -> None:
        """"""
        if not setting:
    ...
    def add_bar_recording(self, vt_symbol: str) -> None:
        ...
        if vt_symbol in self.bar_recordings:
            self.write_log(f"Already in bar recording: {vt_symbol}")
            return

        if Exchange.LOCAL.value not in vt_symbol:
            contract: ContractData = self.main_engine.get_contract(vt_symbol)
            if not contract:
                self.write_log(f"Cannot find symbol：{vt_symbol}")
                return
        ...
        self.write_log(f"Add to bar recording success: {vt_symbol}")

    def add_tick_recording(self, vt_symbol: str) -> None:
        """"""
        if vt_symbol in self.tick_recordings:
            self.write_log(f"Already in tick recording: {vt_symbol}")
            return
        ...
        self.write_log(f"Add to tick recording success:{vt_symbol}")

    def remove_bar_recording(self, vt_symbol: str) -> None:
        """"""
        if vt_symbol not in self.bar_recordings:
            self.write_log(f"Not in bar recording: {vt_symbol}")
            return
        ...
        self.write_log(f"Remove bar recording success: {vt_symbol}")

    def remove_tick_recording(self, vt_symbol: str) -> None:
        """"""
        if vt_symbol not in self.tick_recordings:
            self.write_log(f"Not in tick recording: {vt_symbol}")
            return
        ...
        self.write_log(f"Remove tick recording success: {vt_symbol}")
